# Windows Temelleri - Ana Harita

Bu dosya, Windows işletim sisteminin temel yapısı, mimarisi ve güvenlik mekanizmaları üzerine aldığım notların ana merkezidir. İlgili konuya gitmek için aşağıdaki bağlantılara tıklayabilirsin.

## Windows Giriş
* [[Windows Tarihçesi ve Sürümleri]]
* [[Windows'a Erişim Sağlama (Yerel ve RDP)]]
* [[Windows Arayüzü ve Bileşenleri]]

## Windows Mimarisi Temelleri
* [[Windows Mimarisi]]
* [[Windows Sistem Klasörleri ve Dizin Yapısı]]
* [[FAT ve NTFS Dosya Sistemleri Karşılaştırması]]
* [[Windows Gölge Kopya (Shadow Copy)]]
* [[Alternatif Veri Akışları (ADS)]]
* [[Kullanıcı Yapısı (Yöneticiler ve Standart Kullanıcılar)]]
* [[Erişim Kontrol Listesi (ACL)]]
* [[Kullanıcı Hesabı Denetimi (UAC)]]

## Windows Sistem Yönetimi ve İzleme
* [[Windows Hizmetleri ve İşlemleri]]
* [[Görev Yöneticisi (Task Manager)]]
* [[Kaynak İzleyici (Resource Monitor)]]
* [[Sysinternals Suite ve Araçları]]

## Windows Yapılandırma ve Ayarlar
* [[Ayarlar ve Denetim Masası]]
* [[Sistem Yapılandırma (MSConfig)]]
* [[Kayıt Defteri Düzenleyicisi (Regedit)]]
* [[Yerel Grup İlkesi ve GPO]]

## Windows Komut Satırı Araçları 
* [[Komut Satırı Arayüzü (CLI)]]
* [[Komut İstemi (CMD)]]
* [[PowerShell]]
* [[CMD ve PowerShell Karşılaştırması]]
* [[PowerShell Kurulumu]]
* [[Temel CMD Kuralları]]
* [[PowerShell Temelleri ve Yardım Komutları]]
* [[PowerShell Dosya ve Dizin İşlemleri]]
* [[PowerShell Pipeline ve Filtreleme]]
* [[PowerShell Sistem ve Hizmet Yönetimi]]
* [[Active Directory ve RSAT Temelleri (PowerShell)]]
* [[PowerShell ile Yerel Kullanıcı ve Grup Yönetimi]]
* [[PowerShell Ağ ve Bağlantı Komutları]]
* [[PowerShell ile Sistem ve Bilgi Toplama]]
* [[PowerShell Betik Yazımı (Scripting) ve Modüller]]



## Windows Cihaz ve Veri Güvenliği
* [[Microsoft Defender Çekirdek Yalıtım (Core Isolation)]]
* [[Güvenlik İşlemcisi (TPM)]]
* [[BitLocker Sürücü Şifrelemesi]]
* [[Windows Hello ve Biyometrik Doğrulama]]
* [[FIDO2 Güvenlik Anahtarları]]

## Windows İşletim Sistemi Güvenliği
* [[Windows Güncelleştirmeleri]]
* [[Windows Virüs ve Tehdit Koruması]]
* [[Windows Güvenlik Duvarı ve Ağ Koruması]]
* [[Uygulama ve Tarayıcı Denetimi]]




